...
- **Massively reduced VRAM usage via Block Texture Compression (BC7/BC1/BC4/BC5)** — Textures are compressed off the main thread into native GPU formats as they stream in, freeing up gigabytes of video memory and eliminating texture thrashing in crowded events. Compressed textures are cached locally to disk in a modernized, lockless CoolVL-style cache hierarchy (`0/`–`f/` partitions with zero-contention parallel reads, atomic size tracking, rate-limited access timestamps, and dedicated background thread purging). Worker threads perform fast alpha mask classification and pick-through generation during encode, ensuring cutout alphas and opaque textures route to deferred G-buffer alpha masking (`PASS_ALPHA_MASK`) with hardware depth writes to eliminate transparency sorting flips (e.g. posters or foliage popping through walls, dark interior cavities) and preserve accurate object clicking. Includes configurable quality presets (Ultrafast/Fast/Basic/Slow), an in-memory safety ceiling (`VayuBCTextureCacheMaxPendingSize`) to protect system RAM, and full synchronization with startup and manual cache purges. ([#76](https://github.com/Shadowolf7/Vayu-Viewer/pull/76) / [236aefa71a](https://github.com/Shadowolf7/Vayu-Viewer/commit/236aefa71a), [#82](https://github.com/Shadowolf7/Vayu-Viewer/pull/82) / [2340f4c914](https://github.com/Shadowolf7/Vayu-Viewer/commit/2340f4c914), [#90](https://github.com/Shadowolf7/Vayu-Viewer/issues/90) / [345e7effb7](https://github.com/Shadowolf7/Vayu-Viewer/commit/345e7effb7), [435a5664ff](https://github.com/Shadowolf7/Vayu-Viewer/commit/435a5664ff), [7e816942f8](https://github.com/Shadowolf7/Vayu-Viewer/commit/7e816942f8))
- **Instant Startup & VRAM-Based GPU Tiering** — Disables the slow, noisy startup GPU memory bandwidth benchmark by default (`SkipBenchmark = true`), eliminating the Windows duplicate-window launch bug (#6153). Instead, automatically and accurately assigns the optimal GPU feature class directly from detected video memory (VRAM) and available system RAM. ([#88](https://github.com/Shadowolf7/Vayu-Viewer/issues/88) / [c14414ec1f](https://github.com/Shadowolf7/Vayu-Viewer/commit/c14414ec1f))
...
- **UI Audio Frame-Hitch Prevention** — Pins frequently used UI and notification audio buffers (incoming IM sessions, chat alerts) into memory and expands audio buffer headroom from 140 to 180 channels. This exempts notification sounds from aggressive 30-second LRU purging and eliminates main-thread frame freezes caused by synchronous WAV file decoding from disk during incoming messages. ([2d94de81b9](https://github.com/Shadowolf7/Vayu-Viewer/commit/2d94de81b9))
...
- **Stable Mouselook in Vehicles (Horizon Decoupling)** — When driving or flying in mouselook, your camera stays level with the horizon rather than tilting wildly with vehicle roll/bank, while heading and pitch track smoothly. Uses orthonormal vector projection (`LLAgent::getRollFreeRotation()`) instead of Euler angle decomposition to prevent gimbal lock and heading drift when banking at steep angles, and automatically engages only when seated on physical vehicles or actively grabbing driver controls (`isSeatedOnVehicle()`). ([#33](https://github.com/Shadowolf7/Vayu-Viewer/pull/33) / [403b227539](https://github.com/Shadowolf7/Vayu-Viewer/commit/403b227539), [47152caf8d](https://github.com/Shadowolf7/Vayu-Viewer/commit/47152caf8d))
...
- **Configurable Region-Crossing Prediction (Animats / Firestorm FIRE-29021)** — Choose how your vehicle behaves during sim crossings under Preferences → Move → General (*At region crossing*):
  - *Predict trajectory (adaptive)* (default): Employs a Kalman-style low-pass velocity filter (`fsregioncross`) to measure transient physics noise against a smoothed baseline. Automatically clamps ground vehicles on bumpy roads to a safe ~0.1s envelope before 45× Havok collision bounces can fling them into space or underground, while preserving full 1.0s smooth glides for sailboats and aircraft on calm water/air.
...
- **Automated Post-Crossing Visibility & Attachment Recovery (Henri Beauchamp / Cool VL Viewer)** — Eliminates the classic issue of invisible avatar attachments, missing mesh, or desynchronized object linksets after sim crossings or teleports. The viewer automatically triggers a background scene visibility refresh and attachment re-synchronization (`AutoRefreshAttachmentsInSL`) after a brief stabilization delay (default 2s, configurable via `VisibilityAutoRefreshBorder` and `VisibilityAutoRefreshFarTP`, with optional login refresh `VisibilityAutoRefreshLogin`). ([1d4d3bf709](https://github.com/Shadowolf7/Vayu-Viewer/commit/1d4d3bf709))
...
- **High-Performance Disk, Asset & Block Compressed Texture Caches (`LLDiskCache` & `VayuBCTextureCache`)** — Modernized dual-cache architecture with lockless atomic size tracking, hierarchical directory distribution (`0/` through `f/` subdirectories eliminating OS filesystem limits and sluggish directory traversals), eliminated in-memory file indexes for zero-contention parallel reads, background threaded auto-purging at 150% capacity, rate-limited access timestamps (preventing disk write thrashing), safe multi-viewer instance read-only fallback, and full synchronization with startup and on-demand cache purges. ([f3efbe0182](https://github.com/Shadowolf7/Vayu-Viewer/commit/f3efbe0182), [435a5664ff](https://github.com/Shadowolf7/Vayu-Viewer/commit/435a5664ff), [7e816942f8](https://github.com/Shadowolf7/Vayu-Viewer/commit/7e816942f8))
- **Automated Post-Movement Visibility & Attachment Recovery** — Automatically re-synchronizes worn attachments and triggers a scene visibility refresh after region crossings and teleports, resolving ghosted attachments and invisible prims without needing manual relogs or inventory clears. ([1d4d3bf709](https://github.com/Shadowolf7/Vayu-Viewer/commit/1d4d3bf709))
