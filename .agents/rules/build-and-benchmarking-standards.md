...
* **STRICT PROHIBITION: Never initiate ANY build or compilation processes without explicit user clearance.**
  - **Never** invoke `ninja`, `cmake --build`, `make`, or build test targets autonomously (including during research, planning, or unapproved verification).
  - Heavy compilation saturates CPU/memory and can freeze the host workstation, causing data loss.
  - Any build command must be explicitly requested or confirmed by the user in that specific turn.
