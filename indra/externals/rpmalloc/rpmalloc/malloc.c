/* malloc.c  -  Memory allocator  -  2016 Mattias Jansson
 *
 * SPDX-FileCopyrightText: 2016 Mattias Jansson
 ...
 * restrictions. Or, if you choose, you can use it under the MIT license.
 *
 */
...
// Vayu Viewer local patch: RPMALLOC_NO_CXX_OPERATOR_OVERRIDE skips just this
// operator new/delete aliasing (not the plain malloc/free family below) --
// see indra/cmake/RPMalloc.cmake for why: llcommon's Tracy-profiling-aware
// operator new/delete already provides these same mangled symbols in Tracy
// builds, and it calls plain malloc/free internally, so allocations still
// end up in rpmalloc either way. Safe to disable unconditionally beyond
// that too: libstdc++'s own default operator new/delete already call
// malloc()/free() internally, which the aliases below override regardless.
#if !defined(RPMALLOC_NO_CXX_OPERATOR_OVERRIDE)
...
#endif // !RPMALLOC_NO_CXX_OPERATOR_OVERRIDE
